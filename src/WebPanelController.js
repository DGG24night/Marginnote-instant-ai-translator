var __MN_WEB_API_MNInstantAITranslatorAddon = (function () {
  const FRAME_CONFIG_KEY = "mn_web_template_mninstantaitranslatoraddon_frame_config";
  const PANEL_ON_KEY = "mn_web_template_mninstantaitranslatoraddon_panel_on";
  ...
  function resolveWebEntryURL(mainPath) {
    const devServerURL = __MNGetWebDevServerURL_MNInstantAITranslatorAddon();
    if (devServerURL) {
  ...
  function sendBridgeResponse(webView, requestId, result, error) {
    ...
    const script = `window.__MNBridgeReceive_MNInstantAITranslatorAddon('${encodeBridgeJSON(response)}')`;
  ...
  function setupWebPanelUI(controller) {
    ...
    controller.titleLabel.text = "Instant AI Translator";
  ...
  function dispatchBridgeCommand(controller, message) {
    const commandTable = __MN_WEB_BRIDGE_COMMANDS_MNInstantAITranslatorAddon.commands;
  ...
  const panelControllerClass = JSB.defineClass("MNWebPanelController_MNInstantAITranslatorAddon : UIViewController <UIWebViewDelegate>", {
