...
- **AI 问答回答版本切换**：对话中对某条回答「重新回答」后，旧回答不再丢弃——气泡下方操作行左侧出现「‹ 2 / 2 ›」切换器（图标为用户提供的 houtui.svg / qianjin.svg，单色 currentColor 与其他图标一致，disabled 态置灰），可前后翻看同问题的历次回答与对应思考过程；继续追问时以当前展示的版本作为上下文。长按重新回答选模型同样保留版本链；暂停产生的部分回答同样并入版本列表。复制 / 添加笔记 / 继续对话均取当前选中版本。
- **AI 问答历史回放完整上下文**：历史记录仍按「一个问题一条」保存与展示，但每条记录额外保存该轮提问之前的对话快照（context，上限 40 条消息，超出保留最近部分）。点击多轮对话中的某条历史时，回放完整上下文并自动滚动定位到所点条目的提问位置，短暂高亮帮助确认；旧版本（无 context 字段）的历史记录回落为原来的单轮回放。定位到上下文后可继续追问（后续发送全量历史）。
- **小米 MiMo 预置供应商**：设置 → 服务提供商新增「小米 MiMo」预置（baseURL `https://api.xiaomimimo.com/v1`，预置 mimo-v2.6-pro / mimo-v2.6-flash 并标记支持思考）。思考模式按官方文档适配为 `thinking.type: enabled | disabled`——仅开关、无思考等级（「思考强度」选低/中/高均等效开启）；思考内容走 reasoning_content，流式/非流式均已兼容。

### 调整

- **AI 对话界面移除顶部提示文案**：不再显示「AI 对话：基于选中内容继续提问（回车发送）」，对话区域更聚焦；卡片高度测量同步去掉该行。
- **「获取模型列表」自动勾选已添加模型**：拉取到的模型列表中，已添加到该提供商的模型默认打勾（勾选态直接反映当前已添加情况），重复点「添加选中」仍会自动去重不会重复添加。

## v0.9.6（2026-09-18）

### 修复

- **双击图钉无法进入拼接模式**：进入拼接模式原挂在原生 `dblclick` 事件上，但卡片 WebView 里该事件不可靠——双击第一下让按钮获得 DOM 焦点，第二次 `mousedown` 触达卡片自带的 document 级处理（为保住卡片窗口焦点而 blur 当前元素），原生双击序列被打断，`dblclick` 不派发，界面只看到图钉闪一下。现改为在 `click` 上自行判定双击（450ms 判定窗口为主力，另兼容 `e.detail >= 2` 与原生 `dblclick` 两条冗余路径，三路共享 250ms 触发闩锁防重复切换），并新增「长按图钉 0.6 秒」作为兜底入口（与机器人按钮/长按重新生成的长按约定一致，长按后松手的 click 会被抑制）。进入失败不再静默：此前 catch 吞掉错误导致「双击毫无反应」，现在卡片会直接显示插件返回的失败原因。
- **进入拼接模式的窗口上下文兜底**：窗口解析改为多来源（控制器实例属性 / 插件实例属性 / 最近一次划词记录的窗口），任一环节在 bridge 上下文中不可达时不再直接抛「缺少窗口上下文，无法进入拼接模式」；卡片控制器创建与插件实例注入时同步刷新 `addonWindow` / `addon` 引用。

## v0.9.5（2026-09-15）

### 修复

- **彻底修复查词/翻译开关重新开启后划词不响应**（v0.9.4 修复不彻底）：从设置面板重新开启开关时，重启监听的调用链经过 UIWebView URL 拦截回调上下文，任一环节（插件实例属性可达性 / self.window / 定时器调度）静默失败都会让监听保持停止且无任何报错，只能退出重进文档恢复。现改为三层冗余保障，无论哪个环节失效都能恢复：① SelectionMonitor 新增 sync 对账方法，持久化最近窗口与回调（停止后保留），不依赖插件实例即可自行重启；② 配置保存后双路径同步（插件实例 + 直连监听器）互为兜底；③ 打开笔记本时启动 0.5 秒对账心跳（原生生命周期上下文，运行可靠），监听「应运行而未运行」时最迟 0.5 秒自动恢复。另新增文档打开（documentDidOpen）对账钩子；应用重启时两开关全关、之后才开启的边界场景也已覆盖（窗口预先记录，开启后兜底路径可用）。关闭两开关时插件仍保持完全静默（心跳只读内存缓存配置，不轮询选区、不读焦点笔记、不弹诊断提示）。

## v0.9.4（2026-09-12）

### 修复

- **查词/翻译开关重新开启后立即恢复划词监听**：关闭「查词」「翻译」再重新开启时，无需退出并重开笔记本，在文档中直接划词即可恢复查词/翻译。此前恢复判定依赖监听器 isRunning 状态——stop 流程中定时器失效（invalidate）遇到桥接异常时可能残留已失效的 timer，导致 isRunning 恒为 true、恢复分支永远跳过重启。现恢复分支与「打开笔记本」同路径无条件重启监听，并对 SelectionMonitor 做了加固：stop 无论成败都清空 timer；start 时将文档残留选区预置为「已触发」，避免旧选区被超时兜底自动补触发一次翻译。

## v0.9.3（2026-09-10）

### 新功能

- **流式输出打字机平滑**：AI 回复与思考过程在流式期间按固定节拍（33ms 一拍）逐字揭示，步长按积压量与实际间隔弹性追赶，不再因 80ms 合帧推送而整块蹦字；覆盖结果区（正文 + 思考块）与 AI 对话（回答草稿 + 思考块）。常规设置「通用」新增「打字机效果」开关（默认开启），关闭即回到收到多少显示多少；缓存命中 / 历史回放等一次性结果仍立即全文显示，复制 / 建卡 / 笔记 / 存档仍取完整原文。
- **暗色主题下面板顶栏跟随主题**：插件设置面板的原生标题栏（Instant AI Translator 那一条）此前恒为浅色，现随「主题」切换为深色，标题与关闭按钮配色同步调整；设置页/卡片页应用外观配置时经 bridge 命令 applyPanelTheme 同步，面板首次创建与每次显示时按已保存配置兜底。

## v0.9.2（2026-09-01）

### 新功能

- **查词结果显示词态变化**：有道 / 必应 / 海词 / 金山词霸四家词典均解析「复数 / 过去式 / 现在分词」等词态变化（有道兼容移动版与桌面版两种页面结构；金山从 SSR HTML 的 Morphology 区块按前缀匹配解析），在结果卡片释义下方以「标签 + 值」成对展示，自动换行；无该区块的单词不显示。「添加卡片」与复制生成的 Markdown 正文同步附带「词态变化」段（以 --- 分隔）。

## v0.9.1（2026-08-31）

### 修复

- **翻译任务不再提取选区上下文**：划词翻译句子/段落时不再从当前页文本层提取前后文（{context} 恒为空串）。长选区跨行/跨页时在页文本层定位失败率高，频繁弹「未定位到选区」HUD，且该场景与流式请求静默挂起强相关；翻译选区本身即完整原文，无需注入上下文。查词/AI 解释仍正常提取上下文（「原句/分析」段依赖它）。设置页「选区上下文长度」提示文案与 Prompt 模板 {context} 变量说明同步更新（注明仅查词时注入）。

## v0.9.0（2026-08-30）

### 新功能

- **AI 回复真流式输出**：改用 NSURLConnection delegate 通道（SSE 行缓冲解析 + 80ms 合帧推送），AI 解释 / AI 翻译 / 长难句讲解 / AI 问答全部逐字实时显示，打字节奏与真实生成速度一致；HTTP 错误直接报错，传输失败或空回复自动回退一次性请求重发。通用设置「打字机效果」更名「流式输出」（配置项不变），关闭后等待完整结果一次性显示（机器翻译打字机模拟同样受控）。
- **AI 问答暂停按钮**：发送后发送按钮变为暂停按钮（自绘 zanting.svg 图标），点击即取消请求，已生成的部分保留为回答；回答完成或暂停后恢复为发送按钮。
- **AI 思考过程实时显示**：思考型模型（reasoning_content / reasoning 双兼容）思考阶段在折叠块中实时展示——标题「思考中…」滚动跟随、内容超出 180px 转内部滚动；正文开始后自动折叠为「思考过程」，保留可展开回看（手动展开后不会被流式增量压回）；AI 问答中思考内容随回答存档，历史消息默认收起；非流式降级同样展示 message.reasoning_content。卡片高度在思考阶段跟随思考块增长（至折叠块上限）。

### 修复

- **流式静默挂起**：StreamChannel 此前只强引用了 connection，delegate 是局部变量，JS 包装被 GC 回收后回调全部静默丢失（open 之后再无日志）——与 2026-08-06「回调零触发」同根因，现由 session 强引用并在回调丢失时输出诊断日志；另加无响应看门狗：连接 12s 未收到响应头自动取消并降级一次性请求重发（原 45s 等待过长）。
- **思考阶段卡片高度不跟随**：card-fit 布局（100% 高度链 + .card-body 内部滚动）下原测量分支量的 body.scrollHeight 恒等于视口高度，思考块出现后卡片不变长、内容被遮。现纯思考阶段改量 .card-body 的 scrollHeight（完整内容高度），卡片随思考内容同步变大。

### 体验优化

- **上下文定位失败提示时机**：「未定位到选区 / 页文本不可用」的 HUD 从查词瞬间延后到结果出现（或报错）时弹出，不再紧贴流式请求的建立窗口。
- **AI 解释默认 prompt 重写**：新增「专业英语老师」角色设定；音标改为英式/美式分列；释义按词性缩写开头、同一词性不同词义用数字序号分点；原句加粗目标词并附中文翻译；分析段聚焦单词在原句中的释义。
- **双击机器人默认 prompt 重写**：新增英语老师角色设定与「首先以原句开头，并加粗」；结尾新增「你需要讲解的句子是：{text}」引导句（句子主干/结构拆解/难点词汇/全文翻译/理解要点结构不变）。

## v0.8.1（2026-08-29）

### 修复

- **重新生成跟随触发时的 prompt**：机器人单击产生的结果，重新生成（单击或长按选模型）仍使用 AI 解释 prompt；双击（长难句解释）产生的结果重新生成仍使用长难句 prompt（含其独立模型路由）。此前重新生成一律回落 AI 解释模板，双击后换模型重新生成会拿到单词解释而非长难句讲解。

### 体验优化

- **选区上下文长度按词计数**：常规设置「选区上下文长度」单位由字符改为词——英文按单词计、中文按字计（与「触发翻译的单词数」同一套计词语义），选区前后各取 N 词、空白不计词。已保存的配置数值沿用（默认 200 现在表示 200 词，中文场景注入的上下文会变长，可按需调小）。
- **AI 解释默认 prompt 重写**：输出结构改为 音标（美式/英式，列表）→ 释义（词性缩写开头，区分 vt./vi.，不遗漏其他含义）→ 原句（从上下文取原句并加粗目标词 + 中文翻译）→ 分析（结合上下文解析本句含义）；移除原「常用词组/例句/相关词汇/词根词缀」段落，新增 {context} 变量。上下文不可用（未开启/提取失败/搜索框查词）时自动移除末尾悬空的「单词所在的上下文如下：」引导行，避免模型对着空段落编造上下文。
- **长难句解释默认 prompt 重写**：原句改为加粗呈现；「句子主干」分点提取（并列句分列各分句主干，复合句先给主句）；「全文翻译」直译/意译分点输出；「结构拆解」「难点词汇」「理解要点」保持原有要求。

## v0.8.0（2026-08-29）

### 新功能

- **AI 对话思考强度（推理档位）**：对话输入行新增「思考」按钮，弹出档位列表，选择持久化（= 上次使用的档位，跨会话记忆）；请求参数按厂商/模型自动适配（豆包/DeepSeek/智谱 `thinking.type`、Kimi `reasoning_effort`、百炼/SiliconFlow 的 deepseek/kimi `reasoning_effort`、通义 `enable_thinking`、Ollama 等其余走 `reasoning_effort`）。仅「设置 → 服务提供商 → 测试」探测确认支持思考的模型可调，避免发出厂商不识别的思考参数导致 400。
- **对话模型直接在对话页选择并记忆**：点击输入行模型按钮切换模型，选择持久化到 chat 路由（= 上次使用的模型）；移除路由页原「AI 对话（长按机器人图标）」设置项。新增「双击机器人图标」模型路由（长难句解释用），留空时使用与单击（AI 解释）相同的配置。
- **快捷键 C 进入 AI 问答**：结果卡片内按 C 与长按机器人图标等效；设置页快捷键区新增「AI 问答（同长按机器人）」，可自定义。
- **「新建对话」按钮**：发送按钮移至输入框右侧，原位置换为「新建对话」（圆底加号图标）——清空当前消息与流式草稿、开始新对话；已保存历史与输入框内容保留，回复进行中禁用。
- **AI 问答历史数量**：设置页可配置上限（0-500，默认 50），达到上限自动删除最旧历史；设为 0 关闭历史记录。
- **长难句解释 prompt 重写**：双击机器人图标按结构化模板输出（句子主干 / 结构拆解 / 难点词汇 / 全文翻译直译+意译 / 理解要点），讲解语言跟随目标语言。

### 修复

- **AI 问答建卡只有选中文本、无标题/内容**：旧实现复用 `highlightFromSelection` 新建的高亮摘录并直接改写其 `noteTitle`/`excerptText`——摘录展示跟随 PDF 锚点，改写不生效，卡片只剩原文。改为「空摘录即卡片」：标题写 `noteTitle`、内容以 `appendMarkdownComment` 附为评论（单节点、原文保持高亮）；选区为已有内容的旧摘录时不改动它、新建卡片挂为其子节点；无选区时建独立文字卡片。
- **建卡进错脑图**：原先按 `dc.notebookId`（文档主脑图）建卡，打开子脑图时卡片建到主脑图。改用 `notebookController.notebookId`（官方文档 NotebookController 页「当前笔记本ID」，即当前打开的脑图），拿不到时才回退主脑图。
- **「保存并创建卡片」静默失败**：bridge 同步命令抛错时回包 requestId 丢失（固定回 "unknown"），前端 Promise 永久挂起、无任何提示。卡片与设置页两个控制器的 catch 均改为从 bridge URL 重解析真实 requestId，失败原因现在会以提示显示（如「保存笔记失败：<原因>」）。
- **笔记编辑面板不跟随卡片缩放、按钮被遮住**：编辑界面改为 flex 布局（面板撑满卡片、textarea 内部滚动、按钮行固定贴底），高度测量改为内容高度钳制 [96, 356]，放大跟随撑满、缩小时内部滚动。
- **双击「添加笔记」后编辑页下方露出对话**：笔记编辑界面打开时隐藏对话面板，退出编辑后恢复（消息保留可继续追问）。

### 体验优化

- **对话输入区重排**：输入框独占一行（更宽），下方按钮行为 模型 / 思考 / 新建对话 / 关闭。
- **图标更换**：发送用 fasong.svg（单色 currentColor），关闭复用结果工具栏 chahao 图标，新建对话用 xinjian.svg。
- **设置页整理**：「笔记与机器人」分组更名「AI问答」；「笔记附带查词/翻译结果」复选框文案精简（删除灰色说明行）；AI 服务提供商与机器翻译服务列表默认折叠；路由页/Prompt 页标题精简（「单击机器人图标（AI解释）」「双击机器人图标」），变量说明只在「Prompt 模板」下显示一次。
- **SSR 渲染冒烟测试**：新增 `web/smoke-entry.jsx`——vite 生产构建不检查的 const 暂时性死区（TDZ）白屏问题在 renderToString 阶段即暴露（本轮结果卡白屏即由此定位：`chatPanelVisible` 在声明前被引用）。
- **建卡结果提示更准确**：按是否附带原文高亮区分提示（「已添加卡片到当前脑图（原文已高亮）」/「已添加卡片到当前脑图」）。
